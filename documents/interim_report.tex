...
Powered by React, Next.JS allows the use of components to streamline frontend development. These components encapsulate various functionalities and designs, with the benefit of being able to re-use these easily across different pages, such as for the Sidebar that appears on every page after authentication.
\\\\\includegraphics[scale=0.3]{sidebar-outline}
\captionof{figure}{The sidebar here is an example of a component that I have reused across the application}

For each page that requires the use of the sidebar, I would split them into separate constants for example. The actual page content itself would be in one constant, which is then surrounded by the SidebarMenu component to add the sidebar to the page - before being exported for rendering
...
This also makes designing the frontend substantially easier - as it provides the flexibility for me to use components made freely available for public use. This is beneficial for me personally, as this project doubles as an opportunity to learn about frontend development, with usage of these components simplifying the process.
