...
\section{User Login}
Once the user has created their account, they will be redirected to the login page where they are able to enter their details. 
\\\\\includegraphics[scale=0.3]{login}

When submitted, the username and password are packaged into a JSON body to be sent via an axios request to the backend.
...
In the backend, the user details are authenticated, before a JSON Web Token is returned to the frontend as a cookie. This will be used in future API Requests, where the user needs to be authenticated for the functions to be executed.
